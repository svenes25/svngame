...
<html>
	...
	<body>
		...
		<div class="dukkan" style="	clear:both; height:auto;">
			...
			<div class="dukkanadı"><h3 style="color:red" align="center">Çiftlik</h3></div>
				...
				<?php
					...
					if($count1)
					{
						...
						else if($row1['sure']<=$anlık)
						{
							...
							$result=mysqli_query($baglanti,$sql);
							$yeni="select * from urunsat";
							$degisken=mysqli_query($baglanti,$yeni);
							while($satir=mysqli_fetch_assoc($degisken))
							{
								if($_SESSION["kullanici"]==$satir["kullanici_adi"] && $satir["urun"]=="gübre")
								{
									break;
								}
							}
							...
							@$yeniadet=$row1["adet"]+$satir["adet"];
							$Guncelleciftlik= mysqli_query($baglanti,"UPDATE ciftlik SET adet = 0 WHERE kullanici_adi = '$_SESSION[kullanici]' ");
							$Guncelleurun= mysqli_query($baglanti,"UPDATE urunsat SET adet = '$yeniadet', fiyat= '$row1[fiyat]' WHERE kullanici_adi = '$_SESSION[kullanici]' AND urun = '$row1[urun]' ");
							echo "<br><br><a>Şuan Dükkanınız Boş Lütfen Ürün Satın Alın</a>";
							echo "<br><br><a href='urunsatınal.php' class=button>Ürün Satın Al</a>";
						}
						else
						{
							...
							echo "<br><a href='ciftlikfiyat.php' class='button'>Fiyat Güncelle</a><br>";
							echo "<a style='color:red'>Güncelleme İşleminden Sonra Tükenme Bilgileri Değişebilir.</a>";
							
							echo "<br><br><a style='color:black'>Belirlediğiniz Fiyat Üzerinden Süre Sonunda Satışa Sunulacaktır.</a><br>";
							echo "<a style='color:black'>Hali Hazırda Satışta Bulunan Ürünleriniz Varsa Fiyatlar Güncellenecektir.</a><br>";
